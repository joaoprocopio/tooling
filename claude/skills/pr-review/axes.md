...
4. Nothing found, and the user says there is none: the Spec axis runs against the claim alone, and the report says so

The **claim** is what the change says about itself: the PR description when there is one, and the commit messages otherwise.
...
- **Every smell is a judgement call**, labelled as one ("possible Feature Envy"). A documented standard can be breached hard; a smell is a question.
...
- **Feature Envy**: a method that reaches into another object's data more than its own. So: Move the method onto the data it envies.
- **Data Clumps**: the same three or more fields or parameters travelling together. So: Bundle them into the type they are asking to become.
- **Primitive Obsession**: a primitive or string standing in for a domain concept. So: Give the concept its own small type.
- **Repeated Switches**: the same switch or if-cascade on the same type, recurring across the change. So: Replace it with polymorphism, or one map both sites read.
- **Shotgun Surgery**: one logical change forcing an edit in three or more files at once. So: Gather what changes together into one module.
- **Divergent Change**: one file edited for two or more unrelated reasons. So: Split it so each part changes for one reason.
- **Speculative Generality**: abstraction, parameters, or hooks for needs the spec does not have. So: Delete it and inline back until a real need shows.
- **Message Chains**: long `a.b().c().d()` navigation the caller should not depend on. So: Hide the walk behind one method on the first object.
- **Middle Man**: a class or function that mostly delegates onward. So: Cut it and call the real target.
- **Refused Bequest**: a subclass that ignores or overrides most of what it inherits. So: Drop the inheritance and compose instead.
