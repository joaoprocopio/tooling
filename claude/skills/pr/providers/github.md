...
| `thread.list`     | `gh api graphql` on `reviewThreads`, below                                                                                                                      |
...
| `verdict`         | `gh pr review <id> --approve` / `--request-changes -F <verdict.md>` / `gh pr close <id> --comment "..."`                                                        |
| `pr.create`       | `gh pr create --title "..." --base <branch> --head <branch> --body-file <file.md> [--reviewer <handle>]... [--draft]`                                           |
| `pr.update`       | `gh pr edit <id> --title "..." --body-file <file.md> [--base <branch>] [--add-reviewer <handle>]`, and `gh pr ready <id>` / `gh pr ready <id> --undo` for draft |
...
`pr.identity` reads the fields from these paths: target is `baseRefName`, source branch `headRefName`, head SHA `headRefOid`, description `body`. There is no single field for the source repo: `isCrossRepository` true means a fork, and the repo is `headRepositoryOwner.login` joined to `headRepository.name`.
...
Nothing on GitHub maps a piece of text to the line the diff carries, so the adapter computes it. Read the diff for the one file and walk its hunks:

```bash
gh pr diff <id> -- <file>        # or filter the full patch to that file
```
...
`gh pr checks <id>` returns each check with a `state`, a `bucket` of `pass`, `fail`, `pending`, `skipping`, or `cancel`, and a `link`. **Exit code 8 means checks are still pending**, which is a result rather than an error. `--watch` waits for them to finish, and `--required` narrows to the ones that gate the merge.

A failed check reads its log through the run, not the PR:

```bash
gh run list -c <head-sha> -s failure --json databaseId,workflowName
