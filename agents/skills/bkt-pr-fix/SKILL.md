...
argument-hint: "<pr-id…> or a bkt-pr-review report"
disable-model-invocation: true
---

Read the review comments on a PR, settle what they ask for, land the fixes, and close every thread. Call the Skill tool for `bkt-pr` first, which carries checkout, anchors, drift, threads, suggestions, resolution, tasks, and the voice every step below uses. `bkt-pr-review` is the other half of the loop: it writes the threads this skill works.
...
- **A `bkt-pr-review` report**, when the review ran against a bare ref and posted nothing: each finding arrives with a file, a line, a change, and an axis, and carries no thread ID. Steps 2 through 4 run on the findings, and step 5 answers the user instead of the PR.
...
### 1. Read the comments and check out (`bkt-pr`)

Check the PR out first, so steps 4 and 5 have the branch they edit and push.

Read every live thread, general and inline. The general comment carries no anchor and usually names what blocks the merge, so read it as a thread like any other and give it the whole diff as its subject. Run `bkt pr task list` alongside the comments, because a blocker's task ID differs from its thread ID and step 5 completes the task by that ID.

**Done when** every live thread is listed with its thread ID, its anchor, the change it asks for, and any suggestion or task ID hanging off it. Working from a report instead, every finding is listed with its file, line, and change.
...
Change what the thread asks for and stop there; `karpathy-guidelines` governs the edit. Apply an attached suggestion rather than retyping it, previewing it first. Keep one commit per thread, so step 5 can cite a SHA per reply.
...
- **Deferred**: name the issue now tracking the work and the reason it sits outside this PR, then resolve and complete the task.
- **Discussed**: ask the question or contest the claim with the evidence, and leave the thread open for the reviewer.

Read `bkt pr checks` on the pushed head. A build red for the change is step 4's work again, so reopen the thread it belongs to and go back. A build red for a cause outside the diff takes a reply of its own on the general thread naming that cause, which is what closes this step short of green.
