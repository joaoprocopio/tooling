...
Turn the work on the current branch into a pull request. Call the Skill tool for `bkt-pr` first, which carries identity, the pair, comments, checks, and the voice every step below uses. `bkt-pr-review` is what runs next on what this skill opens.

## The target

The **target** is the branch the change merges into, and it is a decision, not a default: `bkt pr create` falls back to the remote's default branch, which is wrong whenever the change **stacks**, sitting on top of another branch still in review. Pass `--target` on every create, taking it from the skill's argument, else from the branch the work was cut from:
...
Then check whether the branch already has a PR, since `bkt pr list` filters by state rather than by source branch:

```bash
bkt pr list --state OPEN --json --jq '.[] | {id, title, source: .fromRef.displayId}'
```

An open PR from this branch means the change is already up: step 5 edits it instead of creating a second one.
...
Write the title and the description to a file, so step 5 passes the body as `--body "$(cat <file.md>)"` rather than fighting shell quoting.

The title is one line naming the change in the repo's own vocabulary, matching how the existing PRs in `bkt pr list` are titled. `writing-for-agents` governs the cut, because agents read PR descriptions too: keep the sentences that change a decision, and phrase each one positively, saying what the change does.
...
### 5. Create the PR (`bkt-pr`)

Push the branch first, because a PR against an unpushed head has nothing to show:

```bash
git push -u origin HEAD
bkt pr create --target <branch> --title "..." --body "$(cat <file.md>)" \
  --reviewer <user> --with-default-reviewers [--draft]
```

For the branch that already had a PR, `bkt pr edit <id> --title ... --body ...` updates it in place, and `bkt pr publish <id>` is what takes a draft to ready.

Read the PR back with `bkt pr view <id> --json`, confirming the target, the reviewers, and the state are the ones step 3 settled. A pair opens both PRs, then edits each description to carry the other's link, since neither link exists until both are created.

Then run `bkt pr checks <id> --wait`. A build red for the change is step 2's work again, so fix it and push; a build red for a cause outside the diff takes a general comment naming that cause, which is what closes this step short of green.
